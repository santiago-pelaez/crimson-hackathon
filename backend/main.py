...
########## SETUP ##########
# Load .env from repo if available
...
print(f"GEMINI_KEY_LOADED: {bool(gemini_api_key)}")
...
def run_security_check(current_user: str, metadata: dict) -> Dict:
    ...
    def _parse_sentry_output(text: str) -> Dict:
        ...
        if m:
            ...
            try: score = int(re.sub(r"[^0-9]","", m.group(2)))
            except: score = 0
    ...
    try:
        ...
        parsed = {"verdict": ("RED" if user.get("score", 0) >= 60 else "GREEN"), "score": user.get("score", 0), "reason": "Autonomous mode: score-based triage."}
    ...
    with _state_lock:
        ...
        if parsed["verdict"] == "RED": system_state["is_locked"] = True
